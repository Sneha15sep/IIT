...
def initialize_general_ocr_model(model_name='microsoft/general_ocr', use_gpu=False):
    device = 0 if use_gpu and is_gpu_available() else -1
    tokenizer = AutoTokenizer.from_pretrained(model_name)
    model = AutoModelForTokenClassification.from_pretrained(model_name)
    ocr_pipeline = pipeline("image-segmentation", model=model, tokenizer=tokenizer, device=device)
    return ocr_pipeline

# Perform OCR with General OCR and return plain text
def perform_ocr_general(image_path, use_gpu=False):
    ocr_pipeline = initialize_general_ocr_model(use_gpu=use_gpu)
    results = ocr_pipeline(image_path)
    extracted_text = ' '.join([res['word'] for res in results])
    return extracted_text

# Extract text with bounding boxes and confidence, return as JSON
def extract_text_json(image_path, use_gpu=False):
    ocr_pipeline = initialize_general_ocr_model(use_gpu=use_gpu)
    results = ocr_pipeline(image_path)
    ...
    for res in results:
        structured_output.append({
            'bounding_box': convert_to_native_type(res['box']),  # Ensure it's converted to a native type
            'text': res['word'],
            'confidence': convert_to_native_type(res['score'])  # Ensure it's converted to a native type
        })
...
def main():
    ...
    if uploaded_file:
        ...
        with st.spinner('🔄 Performing OCR...'):
            extracted_text = perform_ocr_general(tmp_filepath)
    ...
    # Footer
    st.markdown('<div class="footer"> Made with ❤️ by Sneha Agrawal</div>', unsafe_allow_html=True)
